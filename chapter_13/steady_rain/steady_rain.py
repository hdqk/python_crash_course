...
class SteadyRain:
    ...
    def _delete_drops(self):
        """Get rid of drops that have disappeared off the screen."""
        screen_rect = self.screen.get_rect()
        for drop in self.drops.copy():
            if drop.rect.top >= screen_rect.bottom:
                self.drops.remove(drop)
        # print(len(self.drops))  # Shows that the drops are being deleted
    ...
    def _create_rain_row(self):
        """Create a single row of rain drops."""
        # Create an drop and keep adding drops until there is no room left.
        # Spacing between drops is one drop width.
        if len(self.drops) < self.settings.drop_limit:
            drop = Drop(self)
